...
1. Usage: the codes can be used to train our models, output results on the testing set, output positional attention weights for chosen group of peptides. And also can be used to test on single hla-peptide pair input from the command line.
...
7. Test on single hla-peptide pair input from the command line.
e.g. 1) in the folder "codes", run in the comnand line:
        python single_test.py  HLA-A*02:01 QIDVEKKIV
     2) then the program will direcrtly output the prediction result of the hla-peptide pair in the command line.
        e.g. the predicted IC50 value is XXXX